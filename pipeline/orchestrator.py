...
class NestedCVOrchestrator:
    def __init__(self, data_path, output_path, target_metals, max_features=None, export_outputs=True):
        self.data_path = data_path
        self.output_path = output_path
        self.target_metals = target_metals
        self.max_features = Config.SHARED_MAX_FEATURES if max_features is None else int(max_features)
    ...
    def run(self):
        ...
        for outer_id, (tr_idx, te_idx) in enumerate(outer_cv.split(X_full, Y), start=1):
            ...
            for j, metal in enumerate(self.target_metals):
                ...
                with Timer() as tm:
                    selected_idx, _ = FeatureSelector.select_bands_lasso(
                        X_tr_full, y_tr, max_features=self.max_features
                    )

                Logger.log(
                    f"[FeatureSelection] outer={outer_id} target={metal} "
                    f"selected_count={len(selected_idx)} elapsed={tm.dt:.2f}s"
                )
                _selected_waves, _selected_waves_float = ResultReporter.print_selected_bands(
                    selected_idx, wavelength_cols, metal
                )
                ...
                X_tr_sel, X_te_sel = X_tr_full[:, selected_idx], X_te_full[:, selected_idx]
